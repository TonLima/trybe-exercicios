...
window.onload = () => {
  // Se ativa no momento que a página é carregada.
  btnEntrar.addEventListener("click", () => {
    // escutador de evento 'click' no botão entrar.
    checkLogin(); // chama a função que verifica os dados de login.
  });

  agreementBox.addEventListener("click", () => {
    // escutador de evento 'click' no checkBox
  ...
  textArea.addEventListener("keyup", () => {
    // Se ativa ao digitar na text area.
